import { OcfClipType } from "../../../schemas/log";
...
function parseReel(reel: string): {
  prefix: string;
  numeric: number | null;
  suffix: string;
} {
...
export const ReelsOptionsZod = z
  .object({ mergeRanges: z.boolean().optional() })
  .optional();
...
export function formatReels(
  ...
  if (!options.mergeRanges) return reelsArray;
  const groups = groupReels(reelsArray);
  if (clipsWithoutReel) {
    groups.push(
      `+ ${clipsWithoutReel} other clip${clipsWithoutReel > 1 ? "s" : ""}`
    );
  }
